    ...
    {reports.map((report, index) => (
      <Page style={styles.page} key={index}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.container}>
            <Text style={styles.text}>
              <Text style={styles.label}>Name:</Text> {report.patient}
            </Text>
            <Text style={styles.text}>
        ...
        <View style={styles.doctor}>
          ...
          <Text style={styles.text}> Dr : {report.doctor}</Text>
        </View>
...
const MedicalReport = () => {
  ...
  useEffect(() => {
    const patID = location.state.appId; // Assuming you're passing patID in location.state

    // Fetch data based on patID
    const fetchData = async () => {
        try {
            const authToken = localStorage.getItem('authToken');
            const response = await fetch('http://localhost:8000/api/v2/doctor/apps/history', {
              method: 'POST',
              headers: {
                'Authorization': `Token ${authToken}`,
                'Content-Type': 'application/json'
              },
              body: JSON.stringify({ app_id: patID }) // Assuming patID is already defined in your scope
            });
            ...
            console.log('respondedData:', data);
